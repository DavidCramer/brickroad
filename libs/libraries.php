<div style="padding: 3px 0 10px;">
    <button type="button" onclick="brickroad_addJSLibrary();" class="button"><i class="icon-plus"></i> Add JavaScript Library</button>
    <button type="button" onclick="brickroad_addCSSLibrary();" class="button"><i class="icon-plus"></i> Add CSS Library</button>
</div>
    ...
    if(!empty($Element['_jsLib'])){
        foreach($Element['_jsLib'] as $key=>$var){
            ...
            echo '<div class="librow" id="'.$key.'">';
                ...
                echo '</div>';
                echo '<div class="libItem liblocation" id="row'.$key.'">';
                    ...
                    echo '</select></span>';
                echo '&nbsp;<a class="confirm button" href="#" onclick="jQuery(\'#'.$key.'\').remove();"><i class="icon-remove"></i></a></div>';
...
//vardump($Element);
    if(!empty($Element['_cssLib'])){
        foreach($Element['_cssLib'] as $key=>$var){
            ...
            echo ' <a class="confirm button" href="#" onclick="jQuery(\'#'.$key.'\').remove();"><i class="icon-remove"></i></a><p class="description">Asset slug, URL</p></div>';
        }
    }
...
<script type="text/javascript">
    ...
    function brickroad_addJSLibrary(){

        var rowID = randomUUID();
        jQuery('#jslibraryPane').append('<div id="'+rowID+'" class="librow"><div style="padding:3px 0;" id="row'+rowID+'" class="libItem"><label>Library</label><input type="text" style="width: 450px;" name="data[_jsLib]['+rowID+']" id="jslib'+rowID+'" class="jslib" value=""></div><div id="row'+rowID+'" class="libItem liblocation"><label>Location</label> <select name="data[_jsLibLoc]['+rowID+']" id="header_radio_'+rowID+'"><option selected="selected" value="1">Header</option><option value="2">Footer</option></select>&nbsp;<a onclick="jQuery(\'#'+rowID+'\').remove();" href="#" class="confirm button"><i class="icon-remove"></i></a></div><p class="description">Asset slug, Wordpress script handle or URL</p></div>');
        var newlibs = new Array();
        jQuery('.assetlabel').each(function(){
            newlibs[newlibs.length] = this.value;
        })
    ...
    function brickroad_addCSSLibrary(){
        ...
        jQuery('#csslibraryPane').append('\
            <div id="'+rowID+'" class="librow" style="padding:3px 0;">\n\
                ...
                <a onclick="jQuery(\'#'+rowID+'\').remove();" href="#" class="confirm button"><i class="icon-remove"></i></a><p class="description">Asset slug, URL</p></div>')
