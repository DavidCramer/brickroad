...
class {{_pluginClass}} {
	...
	public function enqueue_admin_stylescripts() {
		...
		if ( in_array( $screen->id, $this->plugin_screen_hook_suffix ) ) {
			...
			if(file_exists(self::get_path( __FILE__ ) .'configs/fieldgroups-'.$slug.'.php')){
				...
				// templated				
				if(file_exists(self::get_path( __FILE__ ) .'assets/css/styles-'.$slug.'.css')){
					wp_enqueue_style( $this->plugin_slug . '-style-' . $slug, self::get_url( 'assets/css/styles-'.$slug.'.css' , __FILE__ ) );
				}elseif(file_exists(self::get_path( __FILE__ ) .'assets/css/styles-'.$slug.'.php')){
					echo "<style type=\"text/css\">\r\n";
						include self::get_path( __FILE__ ) .'assets/css/styles-'.$slug.'.php';
					echo "</style>\r\n";
				}

				if(file_exists(self::get_path( __FILE__ ) .'assets/js/scripts-'.$slug.'.js')){
					wp_enqueue_script( $this->plugin_slug . '-script-' . $slug, self::get_url( 'assets/js/scripts-'.$slug.'.js' , __FILE__ ) , array('jquery'), false, true );
				}
				return;
			}
	...
	public function detect_elements() {
		...
		{{posttype_detector}}{{shortcode_detect}}{{widget_detect}}{{alwaysload_detect}}{{alwaysload_actions}}{{template_detector}}
	}
